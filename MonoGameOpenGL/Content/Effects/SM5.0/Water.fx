...
float3 GetWavePoint(float3 originalPos, float amplitude, float wavelength, float W, float Q, float2 direction){

	float dotD = dot(originalPos.xz, direction);
	float C = cos(wavelength*dotD + time/100);
	float S = sin(wavelength*dotD + time/100);
...
VertexShaderOutput VertexShaderFunction(VertexShaderInput input)
{
	...
	float A1 = 1;	// amplitude
	float L1 = 2;	// wavelength
	float W1 = 2*3.1416/L1; //phase constant
	float Q1 = 0.5;
	float2 D1 = float2(0, 1);
	
	float A2 = 2;	// amplitude
	float L2 = 100;	// wavelength
	float W2 = 2*3.1416/L2;
	float Q2 = 0.5;
	float2 D2 = float2(1, 0);
	
	
	float3 P1 = GetWavePoint(P0,A1,L1,W1,Q1,D1);
	//float3 P2 = GetWavePoint(P0,A2,L2,W2,Q2,D2);
	
	input.Position.xyz = P1;// + P2;
...
float4 pointLight4 = GetPointLightContrbution(Point4Position, input.PositionWorld, normal, Point4Color, Point4FullPowerDistance, Point4FallOffDistance, Point4Intensity);
	...
	return float4(colorAfterFog, 0.5);
};
